...
| AVAIL-01 | Availability combines all required schedule/policy inputs | 2 | **All 10 inputs passed 2026-07-30.** Gated matrix regressions cover publication (with a positive control), opening hours narrower than the shift, full-day closures, replacement hours in and out, qualification revoke/restore, lead time, advance horizon, the cleanup buffer, provider overlap, customer overlap, and chair capacity. Projection-equals-claim is asserted directly: every slot the engine offers is quoted bookable, and a claimed slot leaves the offer set. Live end-to-end suites through the real API passed 14/14, 17/17, and 3/3, plus an owner-provider probe confirming the closed gap. **Input 4 complete:** employment and owner-as-provider both enforced. Q20 was answered on 2026-07-30 (D-028): a provider-enabled owner is bookable by name and through `any`, offered in the projection against the shop own opening hours, and able to run the visit. Verified live — the owner booked at `201`, and the same request on a Sunday returned `409 outside_shop_hours` for a Mon-Sat shop. |
| AVAIL-02 | Provider/customer/chair capacity holds under race | 2 | Passed 2026-07-30. Provider and customer races each yield exactly one winner and one `23P01` (pre-existing). The new chair race puts two customers on two different barbers at a one-chair shop concurrently and yields exactly one success and one `P4026`; a two-chair control accepts the same pair, proving the refusal is the chair count rather than an unrelated conflict. Serialized by a shop-scoped advisory lock ordered before the customer and provider locks (D-023). |
| BOOK-01 | Manual default, optional instant, 15-minute request hold | 3 | **Implementation/automated gate green 2026-08-02; browser pending.** `api_create_booking` selects mode inside the claim transaction, stores customer-scoped idempotency requests, returns the same appointment under concurrent duplicate submit, rejects changed-payload key reuse, creates manual requests with 15-minute expiry, creates instant bookings as confirmed without exposing an intermediate state, and forces restricted customers to manual. Clean replay; matrix 86/86 twice. |
| BOOK-02 | Exact/preferred/any intent and balanced assignment | 2–3 | Rules passed 2026-07-30; browser pending the customer UI. A pre-push review found that the fallback candidate list was never actually ordered — a sub-select’s sort does not survive a UNION ALL — so `any` could pick an arbitrary free provider and retries were not guaranteed to agree. Fixed in `20260730000600` and covered by a regression that pins the eligible set to two providers and was falsified before being trusted. `exact` on a taken slot refuses (`23P01`) instead of substituting. `any` books without naming a barber, resolves the qualified provider with the fewest assigned minutes on the shop's local date — tie-broken by earliest shift start then provider id so a retry agrees — and records `assignment_source = automatic` with a reason. `preferred` tries the named provider first and then substitutes, keeping the requested provider alongside the assigned one; with no alternative it refuses with `P4033`. Owner reassignment records `assignment_source = owner` and keeps its immutable `reassigned` event. Resolution runs inside the claim transaction, so the chosen provider cannot be taken between the decision and the write. |
| BOOK-03 | Owner accept/decline/assign/reassign UI and audit | 3 | **Implementation/automated gate green; browser pending.** Owner Reservations exposes versioned accept/decline and permitted assignment facts. Exact cannot be silently replaced. Versioned provider/service/time proposals require customer response, recheck canonical availability on approval, and preserve the original on reject/conflict. |
| VISIT-01 | Check-in/start/finish/confirm/timeout/dispute lifecycle | 3 | **Implementation/automated gate green; browser pending.** Server-derived allowed actions drive expiring single-use code/manual check-in, start, finish, confirmation, timeout, dispute, and owner resolution; immutable appointment events remain canonical. |
| VISIT-02 | Material service/reassignment change requires consent | 3 | **Implementation/automated gate green; browser pending.** HTTP proposal/response regression proves consent, version/capacity recheck, foreign isolation, and original-preserving conflict. |
| EXC-01 | Cancel, no-show, appeal, strikes, delay, disruption | 3 | **Implementation/automated gate green; browser pending.** Policy/timezone snapshots, late classification, owner/assigned-provider grace guard, seven-day appeal, uphold/waive, rolling restriction, delay, and non-mutating disruption attention are covered. |
| WALK-01 | Staff walk-in, QR/OTP claim, fallback, linked visit | 3 | **Implementation/automated gate green; browser pending.** Public allowlisted claim, durable failed attempts, single-use replay refusal, manual fallback, verified-phone linking, queue events, and direct-write denial pass in the matrix. |
| CLOSE-01 | Closeout is idempotent and never guesses | 3 | **Implementation/automated gate green; browser pending.** Repeating shop/date closeout returns one run; unresolved confirmed visits stay unchanged and receive attention. |
| PAY-01 | Offline collection/correction/refund separate from visit | 3 | **Implementation/automated gate green; browser pending.** Narrow cashier capability, idempotent record, mismatched replay refusal, separate correction/refund/void events, foreign isolation, and direct-write denial pass. |
| NOTIF-01 | Event/outbox atomic; in-app state survives delivery failure | 3–4 | **Implementation/automated gate green; browser pending.** State events enqueue outbox and required in-app facts atomically; provider failure, retry, idempotency, read state, and dead-letter metadata are modeled and matrix-covered. |
