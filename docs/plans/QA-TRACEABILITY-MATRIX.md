...
| AVAIL-01 | Availability combines all required schedule/policy inputs | 2 | **9 of 10 inputs passed 2026-07-30.** Gated matrix regressions cover publication (with a positive control), opening hours narrower than the shift, full-day closures, replacement hours in and out, qualification revoke/restore, lead time, advance horizon, the cleanup buffer, provider overlap, customer overlap, and chair capacity. Projection-equals-claim is asserted directly: every slot the engine offers is quoted bookable, and a claimed slot leaves the offer set. Live end-to-end suites through the real API passed 14/14, 17/17, and 3/3. **Input 4 partial:** employment is enforced; owner-as-provider cannot be booked at all, pending Q20. |
