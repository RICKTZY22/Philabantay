...
| LR-041 | P1 | Raising after incrementing a guest walk-in code's failure counter rolls the increment back, allowing unlimited guesses despite apparently enforced attempts. | Phase 3 public walk-in claim command | P3-05 |
| LR-042 | P1 | A public walk-in claim response could expose internal creator or staff-note fields if it reused the authenticated row projection. | Phase 3 public claim route/projection | P3-05 |
| LR-043 | P1 | A closeout worker that counts or transitions without restricting each command to its target shop can report global work or mutate another shop's due rows. | Phase 3 closeout command loops | P3-08 |
| LR-044 | P1 | Catching every database exception as an ordinary change conflict can hide programming/permission failures and falsely tell the customer the original was safely preserved. | Phase 3 proposal-response command | P3-02 |
...
| LR-041 | Failed claims return a structured result so the attempt update commits; the matrix proves attempts accumulate and lock the claim. | Public HTTP claim regression and replay refusal pass after clean replay. | **Green.** |
| LR-042 | The public route maps an explicit guest allowlist and omits creator/staff notes; authenticated routes retain their scoped projection. | Public HTTP response assertions pass. | **Green.** |
| LR-043 | Closeout expiry/completion loops and counts are filtered by the locked target shop/date; repeat closeout returns the same run. | Two-shop isolation and idempotent closeout regression pass. | **Green.** |
| LR-044 | Only known capacity/precondition codes become a customer-visible change conflict; unexpected database errors propagate and fail the command. | Proposal conflict/success paths pass; code audit confirms the allowlist. | **Green.** |
...
| 2026-07-27 | P2-05 working tree through migration `20260727000300` | Clean migration, owner/barber qualification API, direct-RLS, replay/stale/race, authenticated browser | Verified owner, employed barber, foreign owner, direct authenticated JWT, service role | Complete API suite 59/59 | Capability/qualification/request/event tables and command RPCs denied to authenticated direct callers | Idempotent capability replay passed; stale request and two concurrent qualification sets produced one authoritative winner | No new LR IDs; one reduced-motion transition finding fixed and reverified in-packet | P2-05 green; stop before P2-06 |
| 2026-07-28 | P2-06 Slice 2c through migration `20260728000600` | Clean migration, owner/barber schedule API, direct-RLS, stale/race/P4025, authenticated browser | Verified owning owner, employed barber, foreign/anonymous actors, direct authenticated JWT, service role | Complete API suite 69/69 twice | Direct schedule revision/event writes and direct request-status updates denied; legacy self-write routes/RPCs absent | Concurrent versioned owner writes produced one 200 and one 409; approval atomically linked an exception and revision/event; P4025 count asserted | LR-011 and LR-012 locally closed; LR-026 remains assigned to P2-07 availability/booking integration | P2-06 implementation gate green; independent sign-off pending; P2-07 not started |
| 2026-08-02 | P3-01 through P3-09 working tree, migrations `20260802000100`..`00500` | Clean replay, HTTP/RLS, consent, guest claim, no-show/appeal, payment, notification retry, closeout replay | Anonymous guest, customer, assigned barber, owner, foreign actors, direct authenticated JWT, service role | Complete API suite 91/91 twice | Direct operational writes denied; public guest projection allowlisted | Duplicate booking, proposal conflict, claim replay/attempts, payment replay, outbox retry, and closeout repeat converge | LR-041–LR-044 found and fixed in-packet | Automated Phase 3 gate green; product-owner browser acceptance pending |
